...
def get_local_rl(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        res = array.get_pod_replica_links(local_pod_names=[module.params["name"]])
    if res.status_code != 200:
        return None
    else:
        return list(res.items)[0]
    ...
    if module.params["pause"] is not None:
        if local_rl.status != "paused" and module.params["pause"]:
            ...
            if not module.check_mode:
                if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
                    res = array.pause_pod_replica_link(
            ...
            if not module.check_mode:
                if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
                    res = array.resume_pod_replica_link(
                        local_pod_name=module.params["name"],
                        context_names=[module.params["context"]],
                        remote_pod_name=local_rl["remote_pod_name"],
                    )
...
def create_rl(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        res = list(array.get_array_connections(names=[module.params["target_array"]]))
    if res.status_code != 200:
        module.fail_json(
