...
def get_multi_volumes(module, destroyed=False):
    ...
    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
        res = array.get_volumes(names=names, context_names=[module.params["context"]])
    else:
        res = array.get_volumes(names=names)
    if res.status_code == 200:
        return list(res.items)[0]
    return None
...
def create_volume(module, array):
            ...
            if not module.check_mode:
                if context:
                    res = array.post_volumes(
                        ...
                        volume=VolumePost(
                            provisioned=module.params["size"],
                            qos=Qos(bandwidth_limit=module.params["bw_qos"]),
                        ),
                    )
                else:
                    res = array.post_volumes(
            ...
            if not module.check_mode:
                if context:
                    res = array.post_volumes(
                        ...
                        volume=VolumePost(
                            provisioned=module.params["size"],
                            qos=Qos(iops_limit=module.params["iops_qos"]),
                        ),
                    )
                else:
                    res = array.post_volumes(
        ...
        if not module.check_mode:
            if context:
                res = array.post_volumes(
                    names=[module.params["name"]],
                    context_names=[module.params["context"]],
                    volume=VolumePost(provisioned=module.params["size"]),
                )
            else:
                res = array.post_volumes(
                    names=[module.params["name"]],
    ...
    if module.params["promotion_state"]:
        ...
        if not module.check_mode:
            ...
            if res.status_code != 200:
                if module.params["context"] and context:
                    array.patch_volumes(
                        names=[module.params["name"]],
                        context_names=[module.params["context"]],
        ...
        if not module.check_mode:
            if context:
                res = array.patch_volumes(
                    names=[module.params["name"]],
                    context_names=[module.params["context"]],
                    add_to_protection_groups=FixedReference(
...
def create_multi_volume(module, array, single=False):
    ...
    if bw_qos_size != 0 and iops_qos_size != 0:
        ...
        vols = VolumePost(
            provisioned=human_to_bytes(module.params["size"]),
            qos=Qos(iops_limit=iops_qos_size),
            subtype="regular",
        )
    elif bw_qos_size != 0 and iops_qos_size == 0:
        vols = VolumePost(
            provisioned=human_to_bytes(module.params["size"]),
            qos=Qos(bandwidth_limit=bw_qos_size),
            subtype="regular",
        )
    if not module.check_mode:
    ...
    if module.params["pgroup"] and DEFAULT_API_VERSION not in api_version:
        if not module.check_mode:
            if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
                ...
                res = array.post_protection_groups_volumes(
                    context_names=[module.params["context"]],
                    group_names=[module.params["pgroup"]],
                    member_names=names,
                )
            if res.status_code != 200:
                module.warn(
            ...
            if DEFAULT_API_VERSION not in api_version:
                if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
                    res = array.post_volumes(
                        names=[module.params["name"]],
                        volume=VolumePost(
                            source=Reference(name=module.params["target"])
                        ),
                        context_names=[module.params["context"]],
...
def update_volume(module, array):
    ...
    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
        vol = list(
            array.get_volumes(
                names=[module.params["name"]], ccontext_names=[module.params["context"]]
            ).items
        )[0]
    else:
    ...
    if not hasattr(vol_qos, "bandwidth_limit"):
        vol.qos.bandwidth_limit = 0
    if not hasattr(vol_qos, "iops_limit"):
        vol.qos.iops_limit = 0
                ...
                if not module.check_mode:
                    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
                        res = array.patch_volumes(
                            names=[module.params["name"]],
                            context_names=[module.params["context"]],
                            volume=VolumePatch(provisioned=module.params["size"]),
                        )
                    else:
                        res = array.patch_volumes(
                            names=[module.params["name"]],
...
def main():
    ...
    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
        # If no context is provided and the array is part of a fleet
        # set the context to the local array name
        if not module.params["context"] and array.get_fleets().status_code == 200:
