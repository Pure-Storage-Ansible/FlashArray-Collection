...
def _is_cbs(array, is_cbs=False):
    ...
    #    model = list(
    #        array.get_controllers(context_names=[module.params["context_name"]]).items
    #    )[0].model
    # else:
...
def _set_host_initiators(module, array):
    ...
    if module.params["nqn"]:
        if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
            res = array.patch_hosts(
                ...
                context_names=[module.params["context_name"]],
            )
        else:
            res = array.patch_hosts(
                names=[module.params["name"]],
    ...
    if module.params["iqn"]:
        if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
            res = array.patch_hosts(
                names=[module.params["name"]],
                host=HostPatch(iqns=[module.params["iqns"]]),
                context_names=[module.params["context_name"]],
            )
        else:
            res = array.patch_hosts(
        ...
        if module.params["preferred_array"] == ["delete"]:
            ...
            if not module.check_mode:
                if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
                    res = array.patch_hosts(
                        names=[module.params["name"]],
                        host=HostPatch(preferred_array=[]),
                        context_names=[module.params["context_name"]],
                    )
                else:
                    res = array.patch_hosts(
            ...
            if not module.check_mode:
                if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
                    res = array.patch_hosts(
                        ...
                        context_names=[module.params["context_name"]],
                        host=HostPatch(
                            preferred_arrays=module.params["preferred_array"]
                        ),
                    )
                else:
                    res = array.patch_hosts(
...
def get_multi_hosts(module, array):
    ...
    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
        return bool(
            array.get_hosts(
                names=hosts, context_names=[module.params["context_name"]]
            ).status_code
            == 200
        )
    else:
...
def get_host(module, array):
    ...
    if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
        res = array.get_hosts(
            names=[module.params["name"]], context_names=[module.params["context_name"]]
        )
    else:
    ...
    if not module.check_mode:
        ...
        if LooseVersion(CONTEXT_API_VERSION) <= LooseVersion(api_version):
            res = array.post_hosts(
                names=hosts, host=host, context_names=[module.params["context_name"]]
            )
        else:
