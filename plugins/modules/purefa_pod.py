...
def get_pod(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        return bool(array.get_pods(names=[module.params["name"]], destroyed=False))
...
def get_undo_pod(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        return bool(array.get_pods(names=[module.params["name"] + ".undo-demote.*"]))
...
def get_target(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        return bool(array.get_pods(names=[module.params["target"]], destroyed=False))
...
def get_destroyed_pod(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        return bool(array.get_pods(names=[module.params["name"]], destroyed=True))
...
def get_destroyed_target(module, array):
    ...
    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
        ...
        return bool(array.get_pods(names=[module.params["target"]], destroyed=True))
        ...
        if current_failover == [] or sorted(module.params["failover"]) != sorted(
            ...
            if not module.check_mode:
                if module.params["failover"] == ["auto"]:
                    ...
                    failovers = []
                    for fo_array in range(0, len(module.params["failover"])):
                        failovers.append(Reference(module.params["failover"][fo_array]))
                    if LooseVersion(CONTEXT_VERSION) <= LooseVersion(api_version):
                        res = array.patch_pods(
                            names=[module.params["name"]],
                            context_names=[module.params["context"]],
